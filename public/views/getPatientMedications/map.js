function(doc) {
	if(doc.doctype == "currentMedications" && doc.user_id && (doc.update_ts || doc.insert_ts)){
		...
		if(doc.drug_stop_date){
			if(doc.update_ts) emit([doc.user_id,0,doc.update_ts],doc)
			else emit([doc.user_id,0,doc.insert_ts],doc)
		}else{
			if(new Date() > ed){
			  if(doc.update_ts) emit([doc.user_id,0,doc.update_ts],doc)
			  else emit([doc.user_id,0,doc.insert_ts],doc)
			}else{
			  if(doc.update_ts) emit([doc.user_id,1,doc.update_ts],doc)
			  else emit([doc.user_id,1,doc.insert_ts],doc)
			}
		}
	}
}